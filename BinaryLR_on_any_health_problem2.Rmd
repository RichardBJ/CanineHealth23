---
title: "R stats for binary logistic regression on ANY HEALTH PROBLEM for the all-owner dataset"
author: "Alex German"
...
m <- glm(Any_Health_Problem ~ Primary_Decision_Maker, data = ml,family = binomial)
...
rocr.pred.mtt = ROCR::prediction(pred.mtt, labels = ml$Any_Health_Problem) #ROCR prediction object
...
## D_AGE Binary logistic regression for HEALTH
As age not linearly associated with logit of outcome, add a natural cubic spline
```{r I94}
# fit binary logit model and store results 'm'
m <- glm(Any_Health_Problem ~ bs(D_Age,degree=1,df=2), data = ml,family = binomial)
# view a summary of the model
summary(m)
# test model fit
with(m, null.deviance - deviance)
